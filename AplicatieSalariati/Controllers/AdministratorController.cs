...
namespace AplicatieSalariati.Controllers
{
    ...
    public class AdministratorController : Controller
    {
        ...
        public ActionResult AprobConcedii()
        {
            ...
            foreach (var manager in concediiViewmodel.Manageri)
            {
                foreach (var concediu in concediiViewmodel.Concedii)
                {
                    if ((manager.CNP == concediu.CNP) && concediu.Confirmat == false)
                    {
                        ConcediileModel con = new ConcediileModel();
                        con.CNP = manager.CNP;
                        con.Nume = manager.Nume;
                        con.Prenume = manager.Prenume;
        ...
        {
            DateAdministratorModel dateAdministratorModel = new DateAdministratorModel();
            if (id == null)
            {
                string nume = User.Identity.GetUserName().Substring(1);
                var dateAdministratorModelList = db.DateAdministratorModels.Where
                    (a => a.Nume.Contains(nume)).ToList();
                dateAdministratorModel = dateAdministratorModelList.First();
            ...
            {
                dateAdministratorModel = db.DateAdministratorModels.Find(id);
                if (dateAdministratorModel == null)
                {
                    return HttpNotFound();
                }
